...
/// Capture the visible part of a top-level window from the already composed
/// root framebuffer. This is used when a GPU-backed client exposes a valid but
/// black XComposite/GLX pixmap.
pub fn captureWindowFromRoot(
    ...
    if (image.*.stride < @as(u32, width) * @as(u32, @intCast(bytes_per_pixel))) return error.ImageCaptureFailed;
...
app_insert = r'''    fn cacheSnapshotForWindow(self: *Self, window_id: x11.xcb.xcb_window_t) bool {
    ...
    fn cacheRootSnapshotForItem(self: *Self, item: *ui.DisplayWindow) bool {
        ...
        if (item.cached_snapshot) |previous| rl.UnloadRenderTexture(previous);
        item.cached_snapshot = rt;
        log.debug("Using root snapshot for black GLX window {x}", .{item.id});
        return true;
    }
