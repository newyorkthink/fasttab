...
- Zen Browser 图标识别未解决，曾错误显示 Firefox 图标。现明确将应用类名为 `zen` / `zen-browser` 的小图标留空，保留窗口预览、标题和切换功能；不继续尝试修复其图标来源。
...
- 图标优先匹配窗口实例名和应用类名对应的宿主 desktop/icon；失败后按窗口 PID 依次读取 AppImage 内嵌图标、目标进程根目录中的容器/RunImage desktop/icon，最后回退到 `_NET_WM_ICON` 与 `WM_HINTS`。
