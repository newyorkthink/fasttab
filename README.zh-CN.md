...
FastTab 2.0.4 修复 GPU 渲染类 X11 客户端的黑屏预览，并保留此前的 Firefox 与 i3 修复：

- Microsoft Edge/Chromium 视频和 Remmina 的 XComposite 画面为黑色时，改为捕获已合成的根窗口画面。
- 在 FastTab 显示前立即刷新这类兼容预览，保留当前可见画面。
- 修复 Firefox 合成画面 Alpha 通道为 0 或未定义时预览透明的问题。
- 在 i3 切换工作区前保存当前活动窗口，避免 Code Desktop、Antigravity、Vivaldi 等窗口反复切换后丢失预览。
- 退出切换器后按帧生成缓存预览，避免松开 `Alt/Win` 时卡顿。
- Nerd Font、Powerline 等私有区标题符号会转换为普通分隔符，不再显示为 `?`。
- `Alt+Tab` 显示全部窗口，`Win+Tab` 仅显示当前工作区窗口。
- 同时提供 x86_64 与 ARM64 的 AppImage、DEB 和 RPM 安装包。
...
- Edge/Chromium 与 Remmina 黑屏预览兼容捕获。
...
ARM64 设备请安装 `fasttab_2.0.4_arm64.deb`。
...
需要 Zig 0.14.0 或更高版本、Python 3、C 编译工具链、`make`、`curl`、`tar`，以及 CI 工作流中列出的 X11/OpenGL 开发库。
...
FastTab 以守护进程方式运行，持续跟踪 X11 窗口并维护 GLX 缩略图。Edge/Chromium 与 Remmina 若提供黑色合成画面，则在切换器显示前从已合成的根窗口捕获可见区域并上传为普通 OpenGL 纹理；界面由 Raylib/OpenGL 渲染。
